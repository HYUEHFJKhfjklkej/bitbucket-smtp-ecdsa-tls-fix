#!/usr/bin/env bash
# Read-only SMTP/TLS diagnostic — run on the CONFLUENCE host (working reference) as root.
# Touches nothing, no restart. Saves /tmp/diag-confluence.txt.
# Override defaults via env, e.g.:  RELAY=msk.elara.ru PORT=587 bash diag-confluence.sh
RELAY="${RELAY:-msk.elara.ru}"
PORT="${PORT:-587}"
JRE="${JRE:-/opt/atlassian/confluence/jre}"
{
echo "===== CONFLUENCE diag $(date) @ $(hostname) ====="
...
} 2>&1 | tee /tmp/diag-confluence.txt
