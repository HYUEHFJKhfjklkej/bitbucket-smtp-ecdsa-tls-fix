...
# 3a. архив старой JRE ВНЕ папки установки (на случай падения диска/каталога)
if [ -d "$BB_JRE" ] && [ ! -L "$BB_JRE" ]; then
  ARCHIVE="/root/jre-backup-${TS}.tar.gz"
  say "Архивирую старую JRE в $ARCHIVE"
  tar -czf "$ARCHIVE" -C "$BB_INSTALL" jre && ok "архив создан: $(du -h "$ARCHIVE" | cut -f1) $ARCHIVE"
fi

# 3b. переименование старой JRE на месте + установка новой
if [ -L "$BB_JRE" ]; then
